...
# Define columns considered as personal details
personal_details_columns = ['Client Name', 'Client e-mail', 'Profession', 'Education', 'Country']

# Test case to check input data privacy
def test_input_data_privacy():
    # Check if any personal details columns are present in input_features
    input_features = df.drop(columns=personal_details_columns, errors='ignore')
    
    # Assert that only expected columns are present
    expected_columns = set(df.columns) - set(personal_details_columns)
    assert set(input_features.columns) == expected_columns
    
    print("Input data privacy test passed successfully.")

# Run the test case
test_input_data_privacy()

# Test case to check output data privacy
def test_output_data_privacy():
    # Check if output_feature is correctly named and contains expected data
    assert output_feature.name == 'Net Worth'
    
    print("Output data privacy test passed successfully.")

# Run the test case
test_output_data_privacy()


# Test case to verify dataset shape and column contents
def test_dataset_shape():
    # Define expected columns after dropping personal details
    expected_columns = set(df.columns) - set(personal_details_columns)
    
    # Assert that df contains only expected columns
    assert set(df.columns) == expected_columns
    
    print("Dataset shape verification test passed successfully.")
